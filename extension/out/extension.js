            ...
            if (!fs.existsSync(newDir)) {
                ...
                try {
                    ...
                    if (fs.existsSync(oldFile) && fs.existsSync(newFile)) {
                        ...
                        const existingIds = new Set(newData.sessions.map((s) => s.id));
                        let merged = false;
...
function activate(context) {
    ctx = context;
    pendingOnline = ctx.globalState.get('pendingOnline', []);
    migrateLocalFolders();
    ...
    refreshBars();
    ctx.subscriptions.push(vscode.commands.registerCommand('devCodeTracker.openReport', cmdOpenReport), vscode.commands.registerCommand('devCodeTracker.configure', cmdConfigure), vscode.commands.registerCommand('devCodeTracker.syncNow', () => onlineSync(true)), vscode.commands.registerCommand('devCodeTracker.renameProject', cmdRename), vscode.commands.registerCommand('devCodeTracker.showToday', cmdShowToday), vscode.commands.registerCommand('devCodeTracker.clearData', cmdClearData), vscode.commands.registerCommand('devCodeTracker.showLocalDashboard', cmdShowLocalDashboard));
    ctx.subscriptions.push(vscode.workspace.onDidChangeConfiguration(e => {
        if (e.affectsConfiguration(`${EXT}.idleTimeoutMinutes`)) {
            const minutes = cfg().get('idleTimeoutMinutes') ?? 5;
    ...
    ctx.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(e => { if (e)
        touch(e); }), vscode.workspace.onDidSaveTextDocument(() => touch()), vscode.workspace.onDidChangeTextDocument(e => { if (e.contentChanges.length)
        touch(); }), vscode.window.onDidChangeWindowState(s => {
        if (s.focused)
            startProjectTracking(vscode.window.activeTextEditor);
    ...
    if (!ctx.globalState.get('welcomed')) {
        ...
        vscode.window.showInformationMessage('👋 Dev Code Tracker installed! Sessions save locally. Want to also sync to an online server?', 'Configure API', 'Later').then(choice => {
            if (choice === 'Configure API')
                cmdConfigure();
        });
    }
}
...
function idleMs() { return (cfg().get('idleTimeoutMinutes') ?? 5) * 60000; }
function displayName(folderName) {
    const map = cfg().get('projectDisplayNames') ?? {};
    return map[folderName] ?? folderName;
}
async function saveDisplayName(folderName, name) {
    const map = cfg().get('projectDisplayNames') ?? {};
    map[folderName] = name;
    return safeCfgUpdate('projectDisplayNames', map);
}
async function safeCfgUpdate(key, value) {
    try {
        ...
        vscode.window.showErrorMessage(`Dev Code Tracker: Cannot save settings — your VS Code settings.json has a JSON error. Fix it first.`, open).then(choice => {
            if (choice === open)
...
function computeTodayBaseline(projectPath, _projectKey) {
...
function startProjectTracking(editor) {
...
function endSession() {
    ...
    if (dur >= 60) {
        const s = {
            ...
            date: new Date(snap.start_time.getTime() + 330 * 60000).toISOString().slice(0, 10),
        };
...
function localRead(projectPath) {
...
function localSave(s, projectPath) {
    const data = localRead(projectPath) ?? {
    ...
    if (dir) {
        try {
            fs.writeFileSync(path.join(dir, DATA_FILE), JSON.stringify(data, null, 2), 'utf8');
        }
        catch (e) {
            console.error('Dev Code Tracker write error:', e.message);
        }
    }
    maybePromptReview();
}
...
function maybePromptReview() {
    if (ctx.globalState.get('reviewDone'))
        return;
    const count = (ctx.globalState.get('sessionsLogged') ?? 0) + 1;
    ctx.globalState.update('sessionsLogged', count);
    if (!REVIEW_MILESTONES.has(count))
        return;
    vscode.window.showInformationMessage('You\'ve been coding with Dev Code Tracker — enjoying it? A quick rating helps others find it.', 'Rate it ⭐', 'Not Now', 'Never').then(choice => {
        if (choice === 'Rate it ⭐') {
...
async function onlineSync(notify) {
    if (!isOnlineConfigured())
        return;
    ...
    if (!pendingOnline.length) {
        if (notify)
            vscode.window.showInformationMessage('Dev Code Tracker: Nothing to sync.');
        if (wasActive)
            startProjectTracking();
        return;
    }
    ...
    try {
        ...
        if (notify)
            vscode.window.showInformationMessage(`Dev Code Tracker: Synced ${n} session(s) ✅`);
    }
    catch (e) {
        if (notify)
            vscode.window.showErrorMessage(`Dev Code Tracker sync failed: ${e.message}`);
    }
    if (wasActive)
        startProjectTracking();
}
function collectUnsyncedLocalSessions() {
    const result = [];
    const folders = vscode.workspace.workspaceFolders ?? [];
    for (const folder of folders) {
...
function markLocalSessionsSynced(ids) {
        ...
        if (changed) {
            ...
            if (dir) {
                try {
                    ...
                    console.error('Dev Code Tracker write error:', err.message);
                }
            }
        }
    }
}
function apiPost(apiUrl, payload) {
...
async function cmdConfigure() {
    ...
    if (apiUrl === undefined)
        return;
    if (!await safeCfgUpdate('apiUrl', apiUrl))
        return;
    ...
    if (apiKey === undefined)
        return;
    if (!await safeCfgUpdate('apiKey', apiKey))
        return;
    const idle = await vscode.window.showInputBox({
        ...
        value: String(c.get('idleTimeoutMinutes') ?? 5),
    });
    if (idle !== undefined) {
        const n = parseInt(idle);
        if (!isNaN(n) && n > 0)
            await safeCfgUpdate('idleTimeoutMinutes', n);
    }
    refreshBars();
...
async function cmdRename() {
    ...
    if (!proj) {
        vscode.window.showWarningMessage('Dev Code Tracker: Open a project folder first.');
        return;
    }
    ...
    if (name?.trim()) {
        ...
        if (!ok)
            return;
        if (activeSession)
            activeSession.display_name = name.trim();
        refreshBars();
...
function readLocalWithLive(projPath, projKey, projName) {
    ...
    if (activeSession && activeSession.project_path === projPath) {
        ...
        if (dur >= 60) {
            ...
            data.sessions.push({
                ...
                end_time: now.toISOString(),
                duration_seconds: dur,
                date: new Date(activeSession.start_time.getTime() + 330 * 60000).toISOString().slice(0, 10),
                synced_online: false,
            });
        }
    }
...
async function cmdShowLocalDashboard() {
    ...
    const panel = vscode.window.createWebviewPanel('devCodeTrackerLocalDash', `Dev Code Tracker — Local · ${liveData.display_name}`, vscode.ViewColumn.One, { enableScripts: true });
...
async function cmdShowToday() {
    ...
    const lines = [
        ...
        '',
        ...sessions.map(s => `- \`${ts(s.start_time)}\` → \`${ts(s.end_time)}\` · **${fmtDur(s.duration_seconds)}**`),
    ];
...
async function cmdClearData() {
    const proj = getProject(vscode.window.activeTextEditor);
    if (!proj) {
        vscode.window.showWarningMessage('Dev Code Tracker: No active project.');
        return;
    }
    const ok = await vscode.window.showWarningMessage(`Delete ALL Dev Code Tracker data for "${proj.name}"?`, { modal: true }, 'Yes, delete it');
    if (ok !== 'Yes, delete it')
        return;
    ...
    if (isOnlineConfigured()) {
        ...
        if (choice?.value === 'online') {
            ...
            if (!proj) {
                ...
                const panel = vscode.window.createWebviewPanel('devCodeTrackerReport', `Dev Code Tracker — Local · ${data.display_name}`, vscode.ViewColumn.One, { enableScripts: true });
                const iconUri = panel.webview.asWebviewUri(vscode.Uri.joinPath(ctx.extensionUri, 'icon.png')).toString();
        ...
        if (!proj) {
            ...
            const data = readLocalWithLive(proj.path, proj.key, proj.name);
            const panel = vscode.window.createWebviewPanel('devCodeTrackerReport', `Dev Code Tracker — ${data.display_name}`, vscode.ViewColumn.One, { enableScripts: true });
            const iconUri = panel.webview.asWebviewUri(vscode.Uri.joinPath(ctx.extensionUri, 'icon.png')).toString();
            panel.webview.html = buildOfflineHTML(data, iconUri);
        }
    }
...
function buildOfflineHTML(data, iconUri) {
    const sessions = [...data.sessions].sort((a, b) => new Date(b.start_time).getTime() - new Date(a.start_time).getTime());
    const byDate = {};
    ...
    const monD = new Date(todayD);
    monD.setDate(todayD.getDate() - dFM);
    const weekStart = monD.toISOString().slice(0, 10);
    let streak = 0;
    {
        const c = new Date(todayD);
        while (byDate[c.toISOString().slice(0, 10)]) {
            streak++;
